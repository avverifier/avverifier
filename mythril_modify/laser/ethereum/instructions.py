...
class AddressAnnotation(Annotation):
    def __init__(self, address, function_name):
        self.address = address
        self.function_name = function_name
        self.uid = next(AddressAnnotation.uid_generator)
...
class Instruction:
    ...
    def calldataload_(self, global_state: GlobalState) -> List[GlobalState]:
        ...
        try:
            #在这里先检测value是否为一个address，然后打上标记
            solver = Solver()
            addr1 = symbol_factory.BitVecSym("address1", 256)        
            solver.add(value == addr1)
            result = solver.check()
            #如果reslut为sat，说明value为一地址
            if result == sat:
                annotation = AddressAnnotation(value, global_state.environment.active_function_name)
                global_state.annotate(annotation)
                value.annotate(annotation)
        except:
            pass
