...
class FuncAnnotation(Annotation):
    def __init__(self, func_name):
        #记录每个func的信息，即是否作为第一个函数调用s
        self.func_name = func_name
        self.has_calldata_flag = True
        self.check_already = False
        self.load_flag = ["SLOAD", "SSTORE", "RETURN", "STOP", "SELFDESTRUCT", "CALLER"]

    def isThisFunc(self, state):
        try:
            state_func = state.environment.active_function_name
            if self.func_name == state_func:
                return True
            else:
                return False
    ...
    def has_calldata(self, state):
        if self.check_already:
            return self.has_calldata_flag     

        if self._check_calldataload(state):
            self.has_calldata_flag = True
            self.check_already = True
        else:
            #证明已经加载完成了
            if any(self._check_opcode(state, opcode) for opcode in self.load_flag):
                self.has_calldata_flag = False
                self.check_already = True
            else:
                self.has_calldata_flag = True
...
class LaserEVM:
    ...
    def exec(self, create=False, track_gas=False) -> Optional[List[GlobalState]]:
        ...
        for global_state in self.strategy:
            ...
            if new_states:
                ...
                for states in self.work_list:
                    func_name = states.environment.active_function_name
                    pc = states.mstate.pc
                    instruction = states.environment.code.instruction_list
                self.pure_calldata(new_states)
    ...
    def pure_calldata(self, new_states):
        if len(self.issue_list) > 0:
            self.work_list = list()
            return
        states = new_states[-1]
        func_name = states.environment.active_function_name
        if func_name != "constructor" and func_name != "fallback":
            if any(isinstance(item, FuncAnnotation) for item in  states.annotations):
                for item in states.annotations:
                    if isinstance(item, FuncAnnotation):
                        annotation = item
                        break

                if not annotation.has_calldata(states):
                    if func_name not in self.pure_funclist:
                        self.pure_funclist.append(func_name)

            else:
                annotation = FuncAnnotation(states.environment.active_function_name)
                states.annotate(annotation)
        
        for i in reversed(range(len(self.work_list))):
            work = self.work_list[i]
            if work.environment.active_function_name in self.pure_funclist:
    ...
    def execute_state(
        ...
        try:
            if global_state.world_state.constraints:
                global_state.world_state.constraints.clear()
    ...
    def get_end_addr(self, state):
        instruction_list = state.environment.code.instruction_list
        current_pc = state.mstate.pc
        current_pc += 2
        flag = ["JUMP", "JUMPI", "SLOAD", "SSTORE", "RETURN", "JUMPDEST"]
        while(current_pc < len(instruction_list)):
            opcode = instruction_list[current_pc]['opcode']
            if opcode in flag:
                current_pc -= 1
                break
            elif current_pc == len(instruction_list)-1:
                current_pc = len(instruction_list)-1
                break
            else:
                current_pc += 1
    ...
    def manage_cfg(self, opcode: str, new_states: List[GlobalState]) -> None:
        ...
        if opcode == "JUMP":
            ...
            for state in new_states:
                ...
                end_addr = self.get_end_addr(state)
                self._new_node_state(state, start_addr=start_addr, end_addr=end_addr)
        elif opcode == "JUMPI":
            ...
            for state in new_states:
                ...
                self._new_node_state(
                    state, JumpType.CONDITIONAL, state.world_state.constraints[-1], start_addr=start_addr, end_addr=end_addr
                )
            ...
            for state in new_states:
                start_addr = state.mstate.pc
                end_addr = self.get_end_addr(state)
                self._new_node_state(state, JumpType.RETURN, start_addr=start_addr, end_addr=end_addr)
        for state in new_states:
            state.node.states.append(state)

    def _new_node_state(
        self, state: GlobalState, edge_type=JumpType.UNCONDITIONAL, condition=None, start_addr=0, end_addr=0
    ) -> None:
        """
        ...
        try:
            ...
            return
        new_node = Node(state.environment.active_account.contract_name, start_addr, end_addr)
        old_node = state.node
        state.node = new_node
    ...
    def _execute_pre_hook(self, op_code: str, global_state: GlobalState) -> None:
        ...
        try:
            ...
            temp_issue_list = self.issue_list[:]
            new_issue = annotation.potential_issues[-1]
            has_add = False
            for issue in temp_issue_list:
                if new_issue.function_name == issue.function_name:
                    has_add = True
                    break
            if not has_add:
                self.issue_list.append(new_issue)
        except:
            pass
